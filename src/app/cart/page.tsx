...
interface PropsFromRedux {
  products: ProductDTO[];
  removeSingleCartItemAction: any;
  incrementCartItem: any;
  decrementCartItem: any;
  totalAmount: number;
  clearShopping: (products: ProductDTO[]) => void;
}
...
const Cart = ({
  ...
  const changeQtyValue = (e: ChangeEvent<HTMLInputElement>) => {
    e.preventDefault();
  };
  return (
    <div className="cart-page section-padding min-h-100">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 gap-4">
          <div className="product-items-wrapper">
            <div className="cart-heading-content">
              <div className="total-cart-info">
                Your Cart ({products?.length > 0 ? products?.length : "0"}{" "}
                items)
              </div>
              <div
                className="clear-cart"
                onClick={() => clearShopping(products)}
              >
                {products?.length > 0 ? (
                  <>
                    <svg
            ...
            {products?.length > 0 ? (
                        ...
                        <tr key={item?.product?.id}>
                          <td>
                            <div className="single-item">
                              ...
                              <div className="product-info">
                                ...
                                <h6>Rating: {item?.product?.rating?.rate}</h6>
                              </div>
                            </div>
                          </td>
                          <td>{item?.product?.price}</td>
                          <td>
                            <div className="quantity-wrapper text-center">
                          ...
                          <td className="text-center">
                            <button
                              onClick={() => {
                                dispatch(
                                  removeSingleCartItemAction(item?.product?.id)
                                );
                              }}
                            >
                              <svg
              ...
              <div className="empty-cart">
                ...
                <p>Add your desired products to your cart</p>
              </div>
            )}
          </div>
        </div>
...
const mapStateToProps = (state: RootState) => {
  ...
  const totalAmount =
    data?.length &&
    data?.reduce(
      (accumulatedTotal: any, cartItem: any) =>
        parseInt(
...
const mapDispatchToProps = (dispatch: Dispatch<any>) => {
  return {
    ...
    incrementCartItem: bindActionCreators(incrementCartItem, dispatch),
    decrementCartItem: bindActionCreators(decrementCartItem, dispatch),
    removeSingleCartItemAction: bindActionCreators(
      removeSingleCartItemAction,
      dispatch
    ),
  };
